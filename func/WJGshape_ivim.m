...
%exp_noise = normrnd(0,0.0005,size(bi_exp));
%exp_noise = normrnd(0,0.005,size(bi_exp));
exp_noise = normrnd(0,0.001,size(bi_exp));
bi_exp_noise = bi_exp + exp_noise;
% plot(bi_exp)
...
for loopi = 1:slice 
    ...
    temp_ivim= (new_mask.*II4*ratio+(new_mask*(1-ratio))).*bi_exp_noise(:,:,loopi)*decay;
